  ...
  if(popM5[Age5 == maxage] > 5) { # male pop at older ages tends to be smaller than female, so use this as criteria for extension
    ...
    for (k in 1:nra) {
      ...
      pop_extM[[k]] <- OPAG(Pop = popM5,
                            ...
                            OAnew = OAnew5)
      # PJ: modified age before %in%
      cvM[[k]] <- sd(diff(pop_extM[[k]]$Pop_out[seq(0, OAnew5, 5) %in% c((redist_ages[k]-10):(redist_ages[k]+5))]))/abs(mean(diff(pop_extM[[k]]$Pop_out[Age5 %in% c((redist_ages[k]-10):(redist_ages[k]+5))])))
      ...
      pop_extF[[k]] <- OPAG(Pop = popF5,
                            ...
                            OAnew = OAnew5)
      # PJ: modified age before %in%
      cvF[[k]] <- sd(diff(pop_extF[[k]]$Pop_out[seq(0, OAnew5, 5) %in% c((redist_ages[k]-10):(redist_ages[k]+5))]))/abs(mean(diff(pop_extF[[k]]$Pop_out[Age5 %in% c((redist_ages[k]-10):(redist_ages[k]+5))])))
